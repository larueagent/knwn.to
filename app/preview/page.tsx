  ...
  return (
    <main className="min-h-screen bg-[#F5F0E8] text-[#1A1A1A]">
      ...
      <div className="px-6 py-12 max-w-2xl mx-auto">
        <Link href="/" className="font-mono text-xs text-[#8B7355] tracking-widest uppercase hover:text-[#6B5A42] transition-colors">
          ← knwn.to
        </Link>
        ...
        <p className="text-[#5C5C5C] leading-relaxed mt-4 max-w-lg">
        ...
        <div className="flex gap-2 mt-8">
          <button
            ...
            className={`px-6 py-3 font-mono text-xs tracking-widest uppercase border-2 transition-colors duration-150 ${
              active === "marcus"
                ? "bg-[#1A1A1A] text-[#F5F0E8] border-[#1A1A1A]"
                : "bg-transparent text-[#1A1A1A] border-[#1A1A1A] hover:bg-[#E8E0D0]"
            }`}
          >
            Marcus Webb
          </button>
          <button
            onClick={() => setActive("dani")}
            className={`px-6 py-3 font-mono text-xs tracking-widest uppercase border-2 transition-colors duration-150 ${
              active === "dani"
                ? "bg-[#1A1A1A] text-[#F5F0E8] border-[#1A1A1A]"
                : "bg-transparent text-[#1A1A1A] border-[#1A1A1A] hover:bg-[#E8E0D0]"
      ...
      <div className="px-6 pb-32 max-w-2xl mx-auto">
        ...
        {/* Document header */}
        <div className="border border-[#D4C5A9] bg-[#EDE8DE] p-6 mb-2">
          <h2 className="font-display text-3xl font-bold mb-1">{athlete.name}</h2>
        ...
        <div className="border-l-2 border-[#8B7355] pl-5 py-1 my-8">
          ...
          <p className="text-sm text-[#5C5C5C] italic mt-3">
        ...
        <Section number="01" title="The Portrait">
          {athlete.portrait.split("\n\n").map((para, i) => (
            <p key={i} className="text-[#1A1A1A] leading-relaxed mb-4 last:mb-0">
              {para}
            </p>
          ))}
        </Section>
          ...
          <ul className="space-y-4">
            {athlete.identity.map((item, i) => (
              <li key={i} className="flex gap-4">
                <span className="text-[#8B7355] mt-1 shrink-0">—</span>
        ...
        <Section number="03" title="Pressure Profile">
          <p className="text-[#1A1A1A] leading-relaxed mb-6">{athlete.pressureNarrative}</p>
          <p className="font-mono text-xs text-[#8B7355] uppercase tracking-widest mb-3">When things go wrong:</p>
          <ul className="space-y-3 mb-6">
            {athlete.pressurePatterns.map((p, i) => (
              <li key={i} className="flex gap-4">
                <span className="text-[#8B7355] mt-1 shrink-0">—</span>
                <p className="text-[#1A1A1A] leading-relaxed">{p}</p>
              </li>
            ))}
          </ul>
          <p className="font-mono text-xs text-[#8B7355] uppercase tracking-widest mb-3">What you're afraid people see:</p>
          <p className="text-[#1A1A1A] leading-relaxed italic">"{athlete.pressureFear}"</p>
        </Section>
        ...
        <Section number="04" title="Relationships & Environment">
          <div className="space-y-5 mb-6">
            <div>
              <p className="font-mono text-xs text-[#8B7355] uppercase tracking-widest mb-2">Who gets it:</p>
              <p className="text-[#1A1A1A] leading-relaxed">{athlete.relationshipGets}</p>
            </div>
            <div>
              <p className="font-mono text-xs text-[#8B7355] uppercase tracking-widest mb-2">Who doesn't:</p>
              <p className="text-[#1A1A1A] leading-relaxed">{athlete.relationshipDoesnt}</p>
            </div>
          </div>
          <p className="font-mono text-xs text-[#8B7355] uppercase tracking-widest mb-3">
            What you haven't figured out how to say to your coach yet:
          </p>
          <div className="border-l-2 border-[#8B7355] pl-5">
            <p className="text-[#1A1A1A] leading-relaxed italic">"{athlete.coachQuote}"</p>
          </div>
        </Section>
        ...
        <Section number="05" title="The Direction">
          <div className="space-y-5 mb-6">
            <div>
              <p className="font-mono text-xs text-[#8B7355] uppercase tracking-widest mb-2">What you said you want:</p>
              <p className="text-[#1A1A1A] leading-relaxed italic">"{athlete.directionWant}"</p>
            </div>
            <div>
              <p className="font-mono text-xs text-[#8B7355] uppercase tracking-widest mb-2">What consistent play looks like for you:</p>
              <p className="text-[#1A1A1A] leading-relaxed">{athlete.directionConsistent}</p>
            </div>
          </div>
        ...
        <div className="py-10 border-t border-[#D4C5A9]">
          <div className="flex items-baseline gap-4 mb-6">
            <span className="font-mono text-xs text-[#8B7355]">06</span>
            <h2 className="font-display text-xl font-bold text-[#1A1A1A]">First Read Metadata</h2>
          </div>
          <div className="relative">
            <div className="space-y-2 select-none blur-[3px] pointer-events-none">
              <p className="font-mono text-xs text-[#5C5C5C]">Intake date: {athlete.date}</p>
              <p className="font-mono text-xs text-[#5C5C5C]">Questions answered: 10/10</p>
              <p className="font-mono text-xs text-[#5C5C5C]">Notable themes: {athlete.themes.join(", ")}</p>
              <p className="font-mono text-xs text-[#5C5C5C]">ACSI indicators: loading...</p>
              <p className="font-mono text-xs text-[#5C5C5C]">Wearable data: not connected</p>
            </div>
            <div className="absolute inset-0 flex items-center justify-center">
              <p className="font-mono text-xs text-[#8B7355] uppercase tracking-widest bg-[#F5F0E8] px-4 py-2 border border-[#D4C5A9]">
        ...
        <div className="mt-16 pt-10 border-t border-[#D4C5A9] text-center">
          <p className="font-mono text-xs text-[#8B7355] uppercase tracking-widest mb-4">
            Ready for yours?
          </p>
          ...
          <p className="mt-4 text-xs text-[#8B8B8B] font-mono">Free. 15 minutes. Yours to keep.</p>
        </div>
